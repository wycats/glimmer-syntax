...
function traversalEqual(node: AST.Node, expectedTraversal: Array<[string, AST.BaseNode]>) {
  ...
  expect(actualTraversal.map((a) => `${a[0]} ${a[1].type}`)).toEqual(
  ...
  expect(nodesEqual, `Actual nodes match expected nodes`).toBe(true);
}
...
describe(`Traversal - visiting paths`, () => {
  test('Basics', function (assert) {
    ...
    traverse(ast, {
      TextNode(node, path) {
        expect(node.chars).toBe('bar');
        expect(path.node).toBe(node);
        expect(describeFullPath(path)).toEqual([
