...
function locEqual(
  ...
  if (!node) {
    expect(node, `expected node to exist`).toBeTruthy();
    return;
  }

  expect(node.loc.describe, message).toEqual(
    `test-module@${startLine}:${startColumn}-${endLine}:${endColumn}`
  );
}

describe('Parser - Location Info', () => {
  test('programs', () => {
    let ast = parse(`
    {{#if foo}}
      {{bar}}
  ...
  test('html elements with nested blocks', (assert) => {
    ...
    if (assertNodeType(div, 'ElementNode')) {
      ...
      if (assertNodeType(ifBlock, 'BlockStatement')) {
        ...
        expect(inverseBlock).toBeTruthy();
        if (inverseBlock) {
          ...
          if (assertNodeType(nestedIfBlock, 'BlockStatement')) {
            ...
            expect(nestedIfInverseBlock, 'has nested inverse block').toBeTruthy();
