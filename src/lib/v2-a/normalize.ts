...
export class BlockContext<Table extends SymbolTable = SymbolTable> {
  ...
  constructor(
    ...
    this.ast = Phase1Builder.top(source);
  }
  ...
  resolutionFor<N extends ASTv1.CallNode | ASTv1.PathExpression>(
    ...
    | {
        resolution: 'error';
        path: string;
        head: { name: string; span: SourceSpan };
      } {
    if (this.strict) {
      ...
      case 'SubExpression': {
        ...
        if (resolution.resolution === 'error') {
          throw generateSyntaxError(
            `You attempted to invoke a path (\`${resolution.path}\`) but ${resolution.head.name} was not in scope`,
            expr.loc
          );
        }
